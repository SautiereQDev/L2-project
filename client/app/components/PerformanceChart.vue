  ...
  <div class="performance-chart">
    ...
    >
      <UIcon
        ...
        class="h-10 w-10 text-gray-300 animate-pulse"
      />
    </div>
    <div v-else-if="error" class="py-10 text-center">
      <UIcon
        name="i-heroicons-exclamation-circle"
        class="h-8 w-8 text-warning-500 mx-auto mb-2"
      />
      <div class="text-gray-500">
        Impossible de charger les données d'évolution
      </div>
      <UButton
        size="sm"
        color="gray"
    ...
    <div
      v-if="chartData && !noData && !loading && !error"
...
function generateSimpleChart(dataPoints: any[]) {
  ...
  baseline.className =
    "absolute bottom-0 w-full h-0.5 bg-gray-200 dark:bg-gray-700";
  ...
  dataPoints.forEach((point, index) => {
    ...
    const valueFormatted = formatPerformanceValue(
      point.value,
      props.disciplineType,
    );
    ...
    tooltip.className =
      "absolute opacity-0 group-hover:opacity-100 bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 pointer-events-none transition-opacity z-10 whitespace-nowrap";
