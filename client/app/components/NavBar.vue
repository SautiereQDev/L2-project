      ...
      <div class="flex justify-between h-16">
        <div class="flex">
          <div class="flex-shrink-0 flex items-center">
            <span class="flex items-center gap-2 text-xl font-bold text-primary-600 dark:text-primary-400 cursor-pointer hover:opacity-80 transition-opacity" @click="navigateTo('/')">
...
class="border-transparent text-gray-600 dark:text-gray-300 hover:border-primary-500 hover:text-primary-600 dark:hover:text-primary-400 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium cursor-pointer transition-all"
    ...
    >
      <div v-show="isOpen" id="mobile-menu" class="sm:hidden absolute w-full bg-white dark:bg-gray-800 shadow-lg border-t border-gray-200 dark:border-gray-700 z-20">
          ...
          <div class="px-4">
            <div class="flex-grow">
              ...
              <div v-if="authStore.isAuthenticated" class="space-y-2">
                <a class="flex items-center text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400 py-2 text-base font-medium cursor-pointer" @click="navigateTo('/profile')">
                  <Icon name="heroicons:user" class="h-5 w-5 mr-3" />
                  Mon profil
                </a>
                <button class="flex items-center w-full text-left text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 py-2 text-base font-medium cursor-pointer border-none bg-transparent" @click="authStore.logout(); router.push('/login')">
                  <Icon name="heroicons:arrow-right-on-rectangle" class="h-5 w-5 mr-3" />
                  Se déconnecter
                </button>
              </div>
              <div v-else class="space-y-3 py-2">
                <a class="flex items-center text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400 py-2 text-base font-medium cursor-pointer" @click="navigateTo('/login')">
                  <Icon name="heroicons:arrow-left-on-rectangle" class="h-5 w-5 mr-3" />
                  Se connecter
                </a>
                <a class="flex items-center justify-center w-full bg-primary-600 hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-600 text-white py-2 px-4 rounded-md text-base font-medium cursor-pointer transition-colors" @click="navigateTo('/register')">
