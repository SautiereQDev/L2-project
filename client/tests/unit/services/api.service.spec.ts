...
describe('ApiService', () => {
  ...
  describe('authenticate', () => {
    it('retourneVraiSiConnexionRéussie', async () => {
      (authService.login as Mock).mockResolvedValueOnce({success: true, data: {token: 'token'}});

      const result = await apiService.authenticate('email@test.com', 'password');

      expect(result).toBe(true);
      expect(authService.login).toHaveBeenCalledWith({email: 'email@test.com', password: 'password'});
    });

    it('retourneFauxSiConnexionÉchoue', async () => {
      (authService.login as Mock).mockResolvedValueOnce({success: false, message: 'Échec'});

      const result = await apiService.authenticate('email@test.com', 'mauvais-mdp');
    ...
    it('lancesUneErreurSiEchecAuthentication', async () => {
      ...
      await expect(apiService.get('/users/1')).rejects.toThrow("Échec de l'authentification");
    });
    ...
    it('gèreCorrectementLesErreursApiStructurées', async () => {
      ...
      await expect(apiService.put('/users/1', {name: 'A'})).rejects.toThrow(/Validation échouée/);
    })
  });
});
