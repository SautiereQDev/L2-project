 ...
 */
final class RecordOutputProvider implements ProviderInterface
{
    ...
    public function provide(Operation $operation, array $uriVariables = [], array $context = []): object|array|null
    {
        ...
        try {
            ...
            if ($state instanceof \Traversable || is_array($state)) {
    ...
    {
        $id = $record->getId();
        if (null === $id) { throw new \LogicException('Record ID is null.'); }
    ...
    private function createMinimalRecordDto(Record $record): RecordOutput
    {
        ...
        $discipline = $record->getDiscipline();
        if (null === $discipline) { throw new \LogicException('Record discipline is null.'); }

        $athlete = $record->getAthlete();
        if (null === $athlete) { throw new \LogicException('Record athlete is null.'); }
        
        $location = $record->getLocation();
        if (null === $location) { throw new \LogicException('Record location is null.'); }
